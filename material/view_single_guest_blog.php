...
if($_SERVER['REQUEST_METHOD'] == "GET"){
		try{
			...
			for($i = 0;  $object = $stmt->fetch(PDO::FETCH_ASSOC); $i++){
				$json['items'][] = array("id"=>$object['BlogID'],"title"=>preg_replace('/\s+/', ' ',nl2br(htmlentities(trim($object['title']),ENT_SUBSTITUTE | ENT_QUOTES | ENT_COMPAT | ENT_XHTML, 'UTF-8'))),
				"description"=>addslashes(preg_replace('/\s+/', ' ',nl2br(trim($object['description'])))),
