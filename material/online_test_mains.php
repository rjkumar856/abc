...
if($_SERVER['REQUEST_METHOD'] == "POST"){
		try{
		    $query = "SELECT * FROM online_test_mains GROUP BY subject ASC";
			...
			for($i = 0;  $object = $stmt->fetch(PDO::FETCH_ASSOC); $i++){
			   $json['subjects'][]=$object['subject'];
			    ...
			    $query_result = "SELECT * FROM online_test_mains WHERE subject='$object[subject]' ORDER BY date_added DESC";
    			$stmt_result = $user->runQuery($query_result);
    			$stmt_result->execute();
			    for($j = 0;  $object_result = $stmt_result->fetch(PDO::FETCH_ASSOC); $j++){
				$json['items'][] = array("id"=>$object_result['id'],"subject"=>$object_result['subject'],"title"=>$object_result['title'],
				"file_url"=> DIR_SYSTEM."assets/files/mains/".$object_result['file_url'],"date_added"=>$object_result['date_added']);
			    }
			    }
