...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if(!empty($_REQUEST)){
		try{
		    ...
		    $question_type = htmlentities(trim($_REQUEST['question_type']));
		    ...
		    if(empty($user_id) || empty($question_id) || empty($question_type) || empty($total_questions)){
			$json = array("response" => 201,"status"=>"error","message"=>"Invalid Data");
			json_enc($json );
			exit();
		    }

			$stmt = $user->runQuery("INSERT INTO online_test_submit_prelims(user_id,question_type,question_id,selected_questions,selected_answers,correct_answers,total_questions,answered_questions,corrected_answers,wrong_answers,percentage,marks,ip) 
			VALUES('$user_id','$question_type','$question_id','$selected_questions','$selected_answers','$correct_answers','$total_questions','$answered_questions','$corrected_answers','$wrong_answers','$percentage','$marks','$ip')");
		    $stmt->execute();
        		$json = array("response"=>200,"status"=>"success","message"=>"Online Test Result added successfully!");
        		json_enc($json);
        		exit();
